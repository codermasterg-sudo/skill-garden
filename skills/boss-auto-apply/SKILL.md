...
description: BOSS直聘自动投递。提供浏览器实例管理（browser.py open/close/port）、搜索岗位（search_jobs.py）、点击「立即沟通」（apply_action.py）三个能力，由 agent 根据用户需求和简历自主判断、决策投递。当用户需要自动投简历、批量打招呼、筛选投递岗位时使用。
---
...
本 skill 提供**操作 BOSS 直聘页面的动手能力**：搜索岗位、点击「立即沟通」。agent 负责全部决策——是否值得投、投几个、节奏怎么控制、记录什么，都根据用户需求自主判断，skill 不替 agent 做决定。
...
- **安全**：端口为系统随机（避开 9222/9223/9229 已知检测点）、仅监听 127.0.0.1、关闭即释放。
...
- 在已有浏览器实例中（CDP 复用 `browser.py open` 打开的实例，有头/无头均可）跳转岗位详情页，真实点击「立即沟通」（BOSS 自动带上默认招呼语）
- **需先打开实例**（运行 browser.py open），未打开则脚本报错提示
...
### 记录（agent 负责）
每次投递后，**必须**把投递记录追加到 `data/applied.md`，一行一条。记录内容由 agent 判断，建议包含：日期、时间（**含秒，用于节奏控制**）、job_id、状态（成功/失败/风控暂停/硬顶），以及用户要求的其他信息（公司、岗位、备注等）：
