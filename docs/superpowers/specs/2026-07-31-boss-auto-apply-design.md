...
**窗口架构与 CDP 安全约束**：
- 浏览器实例由 `browser.py open` 显式打开（`--headless` 参数切换有头/无头），搜索/投递脚本一律通过 CDP 连接复用该实例。
- 调试端口为**系统分配随机端口**（避开 9222/9223/9229 等 BOSS 已知检测点），**仅监听 127.0.0.1**，不对外网/局域网开放。
- 端口仅在浏览器开启期间存在（`data/browser_window.json` 记录），关闭即释放，不常驻。
- 页面 JS 受同源策略限制无法读取端口响应内容，仅可发起连接探测，风险可控。
